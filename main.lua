...
local DesyncButton = MainTab:Button({
    ...
    Callback = function()
        ...
        -- รีเซ็ต replication กลับ ไม่งั้น position ค้างบน server
        task.wait(0.5)
...
local function IsGun(tool)
    ...
    return tool:GetAttribute("reload_time") ~= nil or tool:GetAttribute("AmmoType") ~= nil or FindFireRateAttribute(tool) ~= nil
end
...
-- ===== UI Controls (แต่ละตัวทำงานอิสระ ไม่ต้องกด Enable Gun Mod) =====
...
-- Auto-start gun mod ทันทีโดยไม่ต้องกด toggle
task.spawn(StartAutoMod)
    ...
    while _G.BumpAura do
        ...
        for _, target in CharModule.get_all() do  -- เปลี่ยนจาก Char เป็น CharModule
            if target ~= Character then
                ...
                if hrp and (RootPart.Position - hrp.Position).Magnitude < 100 then
