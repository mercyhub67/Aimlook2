...
oldFire = hookfunction(send.FireServer, function(self, ...)
	...
	if SilentAimEnabled and IsHoldingAllowedGun(args) then
		...
		if target and target.Character and target.Character:FindFirstChild("Head") then
			...
			-- Beam จากหัวเราไปหัวเป้า
			if BeamEnabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head") then
				local myHeadPos, onScreen1 = Camera:WorldToViewportPoint(LocalPlayer.Character.Head.Position)
				local targetHeadPos, onScreen2 = Camera:WorldToViewportPoint(head.Position)
				if onScreen1 and onScreen2 then
					createBeam(
						Vector2.new(myHeadPos.X, myHeadPos.Y),
						Vector2.new(targetHeadPos.X, targetHeadPos.Y)
					)
				end
			end
		end
						...
						pcall(function()
							for _, v in pairs(Players:GetPlayers()) do
								if v ~= Client and v.Character and v.Character:FindFirstChild('HumanoidRootPart') then
									...
									end -- ปิด else ของ if not bg
								end
							end
						end)
						end -- ปิด else ของ if not InventoryViewerEnabled
					end
				end)
			end
		else
...
-- Bump Aura Function
local function GetDistanceFromRootPart(hrp)
